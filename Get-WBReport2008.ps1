...
$users = "backup.user@nrhawv.org" # List of users to email your report to (separate by comma)
$fromemail = "backup.user@nrhawv.org"
$server = "mail.nrhawv.org" #enter your own SMTP server DNS name / IP address here
#endregion
...
$result = $WBSummary.LastBackupResultHR
...
# Email the report
Send-MailMessage -from $fromemail -to $users -subject "$computer  $result Windows Backup Report" -BodyAsHTML -body $HTMLMessage -priority Normal -smtpServer $server
